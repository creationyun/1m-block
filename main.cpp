...
bool host_match(std::string str);
...
int main(int argc, char **argv)
{
	...
	// Read hosts to block
	std::string hostname;
	while (std::getline(readfile, hostname)) {
		// check CRLF or LF
		if (hostname[hostname.size()-1] == '\r') {
			hostname = hostname.substr(0, hostname.size()-1);
		}
	
		block_host.insert(hostname);
	}
...
bool host_match(std::string str)
{
	...
	while (pos != std::string::npos) {
		...
		if (block_host.find(str.substr(pos)) != block_host.end()) {
			return true;
		}
